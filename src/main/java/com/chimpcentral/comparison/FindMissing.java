...
class FindMissing extends Find {
	...
	void processMap(String jsonpath) {
		Iterator<Object> iterator = firstJsonObject.getAsHashMap(jsonpath).keySet().iterator();
	...
	void processList(String jsonpath) {
		...
		Iterator<Object> iterator = firstJsonObject.getAsList(jsonpath).iterator();
		List<Object> secondJsonObjectList = secondJsonObject.getAsList(jsonpath);
		while (iterator.hasNext()) {
			Object value = iterator.next();
			boolean doesValueExistsInSecondJsonObject = secondJsonObjectList.contains(value);
	...
	private boolean findDuplicates(Object value, String jsonpath) {
		List<Object> firstObjectList = firstJsonObject.getAsList(jsonpath);
		List<Object> secondObjectList = secondJsonObject.getAsList(jsonpath);
	...
	private int findNumOfReferences(Object value, List<Object> list) {
	...
	private List<Object> sortList(List<Object> values) {
		Collections.sort(values, (object1, object2) -> object1.toString().compareTo(object2.toString()));
		return values;
	}
